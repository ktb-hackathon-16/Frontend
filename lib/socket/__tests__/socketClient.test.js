...
describe('socketClient', () => {
  ...
  it('sends chat room domain events through the socket service', () => {
    ...
    // [CHANGED] markMessagesAsRead(messageIds, socket) -> markMessagesAsRead(roomId, lastReadMessageId, socket)
  ...
  it('rejects marking messages as read without roomId/lastReadMessageId', () => {
    const service = {
      send: vi.fn(),
    };
    const client = createSocketClient(service);

    expect(() => client.markMessagesAsRead(null, 'message-1')).toThrowError(
      'roomId and lastReadMessageId are required',
    );
    expect(service.send).not.toHaveBeenCalled();
  });
  ...
  it('subscribes room event handlers and returns an unsubscribe function', () => {
    ...
    socket.emitToClient('messagesRead', { userId: 'user-1', roomId: 'room-1', lastReadMessageId: 'message-1', lastReadAt: '2026-08-10T00:00:00.000Z' });
    ...
    expect(handlers.onMessagesRead).toHaveBeenCalledWith({
      ...
      roomId: 'room-1',
      lastReadMessageId: 'message-1',
      lastReadAt: '2026-08-10T00:00:00.000Z',
    });
